...
class GraphDatabase(KnowledgeBase):
    ...
    def _get_me_clause(self, entity_type: Text) -> Text:
        ...
        if entity_type not in ["product", "capstone"]:
            clause = (
                f'$product isa product, has Product_RMIT "{self.capstone["Product_RMIT"]}";'
    ...
    def _get_card_entities(
        ...
        return self._execute_entity_query(
            f"match "
            f"{me_clause} "
            f"$represented-by(Product_RMIT: $account, Segment: $sSegment) "
            f"isa represented-by;"
            f"$product isa product{attribute_clause}; "
            f"get $product;"
        )[:limit]
    ...
    def _get_account_entities(
        ...
        attribute_clause = self._get_attribute_clause(attributes)
        me_clause = self._get_me_clause("Product_RMIT")
